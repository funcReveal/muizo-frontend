...
const RevealChoiceAvatarRow: React.FC<RevealChoiceAvatarRowProps> = ({ picks }) => {
  const visiblePicks = picks.slice(0, VISIBLE_AVATARS);
  const overflowCount = Math.max(0, picks.length - visiblePicks.length);

  return (
    <div className="game-room-choice-avatar-row" aria-label="作答玩家頭像">
