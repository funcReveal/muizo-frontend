...
describe("RoomLobbyHostControls", () => {
  ...
  it("switches between compact source pills and only shows one active source view", () => {
    ...
    expect(screen.getByText("房主控制")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "玩家推薦" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "貼上連結" })).toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: "貼上連結" }));

    expect(
      screen.getByPlaceholderText("貼上 YouTube 播放清單 URL"),
    ).toBeInTheDocument();
  });
  ...
  it("keeps source actions disabled while the game is playing", () => {
    ...
    expect(screen.getByRole("button", { name: "套用到房間" })).toBeDisabled();
  });
});
