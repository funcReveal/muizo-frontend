        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              aria-pressed={active}
              className={`relative min-w-0 border px-2 py-2.5 text-center transition sm:px-3 sm:py-2 ${
                docked ? "rounded-b-[15px] rounded-t-none" : "rounded-[15px]"
              } ${
                active
            ...
            >
              <div className="truncate text-[12px] font-semibold leading-none sm:hidden">
                {tab.shortLabel}
              </div>
              ...
              <div
                className={`mx-auto mt-1.5 h-0.5 rounded-full transition ${
                  active ? "w-7 bg-amber-200 sm:w-8" : "w-0 bg-transparent"
                }`}
              />
            </button>
          );
