...
const resolveSongPerformanceMetrics = (
  ...
  return { rankNorm, speedNorm, difficultyNorm };
};
...
export const resolveSongPerformanceSegments = (
  ...
  if (input.result === "correct") {
    return [
      { key: "base", label: "答對", value: 60, max: 60, color: "#f5b318" },
      { key: "rank", label: "搶答", value: Math.round(rankNorm * 20), max: 20, color: "#5ea4ff" },
      { key: "speed", label: "速度", value: Math.round(speedNorm * 20), max: 20, color: "#2dd4bf" },
      { key: "difficulty", label: "難度", value: 0, max: 0, color: "#f472b6" },
    ];
  }

  if (input.result === "wrong") {
    return [
      { key: "base", label: "作答", value: 8, max: 8, color: "#fb7185" },
      { key: "rank", label: "搶答", value: Math.round(rankNorm * 8), max: 8, color: "#5ea4ff" },
      { key: "speed", label: "速度", value: Math.round(speedNorm * 9), max: 9, color: "#2dd4bf" },
      { key: "difficulty", label: "難度", value: Math.round(difficultyNorm * 5), max: 5, color: "#f472b6" },
    ];
  }

  return [
    { key: "base", label: "作答", value: 0, max: 0, color: "#64748b" },
    { key: "rank", label: "搶答", value: 0, max: 0, color: "#5ea4ff" },
    { key: "speed", label: "速度", value: 0, max: 0, color: "#2dd4bf" },
    { key: "difficulty", label: "難度", value: Math.round(difficultyNorm * 4), max: 4, color: "#f472b6" },
  ];
};
...
export const resolveAutoGuideStartTarget = <T extends { previewUrl: string | null }>(
  rowsByCategory: Record<RecommendCategory, T[]>,
  ...
  for (const category of categorySequence) {
    ...
    if (previewIndex >= 0) return { category, index: previewIndex, hasPreview: true };
  }
  const preferredRows = rowsByCategory[preferredCategory] ?? [];
  if (preferredRows.length > 0) {
    return { category: preferredCategory, index: 0, hasPreview: Boolean(preferredRows[0]?.previewUrl) };
  }
  for (const category of orderedFlow) {
    const rows = rowsByCategory[category] ?? [];
    if (rows.length <= 0) continue;
    return { category, index: 0, hasPreview: Boolean(rows[0]?.previewUrl) };
  }
  return null;
};
...
export const resolveRecapPreviewNavigation = (
  ...
  if (source === "click") {
    return { playbackMode: "idle", forcePreview: false };
  }
...
export const resolveSpeedComparisonInsight = (
  ...
  return { label: "比中位快慢", value: "--", deltaMs: null, answeredMs, averageMs };
};
