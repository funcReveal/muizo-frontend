...
const RequireAuthRoute: React.FC<RequireAuthRouteProps> = ({
  ...
  const { authLoading, authUser, loginWithGoogle } = useAuth();
  ...
  const statusLabel = "尚未登入";
  const statusHint = "登入帳號後即可保存並查看完整歷史。";
  const primaryActionLabel = "使用 Google 登入";
  const secondaryActionLabel = "房間列表";
